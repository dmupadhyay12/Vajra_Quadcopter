...
typedef struct pid_controller {
    ...
    float curr_state; // current process value
    float output; // corrected output
    float setpoint; // target value for this particular controller, determined by the RC input
    ...
    float prev_error; // For derivative term
...
*/
void update_pid_controller(pid_controller_t* controller, float kp_new, float ki_new, float kd_new);
